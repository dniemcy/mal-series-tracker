...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    window.getAnimeDetails = async (animeId: number) => {
      try {
        const modal = document.getElementById('animeModal');
        ...
        if (detailsElement) {
          detailsElement.innerHTML = `
            <p class="error">Error fetching anime details: ${error instanceof Error ? error.message : String(error)}</p>
          `;
        }
      }
    };
        ...
        <div class="error-container">
          <div class="error-content">
            ...
            <p class="error-message">
              Error initializing application: ${error instanceof Error ? error.message : String(error)}
            </p>
